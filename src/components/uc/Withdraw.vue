<template>
    <div class="nav-right">
                            ...
                            <div class="inner-box">
                                <div class="form-group form-address">
                                    ...
                                    <div class="control-input-group">
                                        <Select v-model="withdrawAdress" :placeholder="$t('common.pleaseSelect')">
                            ...
                            <div class="form-group form-fee">
                                ...
                                <div class="input-group">
                                    ...
                                    <!--<Poptip v-else trigger="focus" :content="$t('uc.finance.withdraw.tip1')+currentCoin.minTxFee+$t('uc.finance.withdraw.tip1')+currentCoin.maxTxFee" style="width: 100%;">-->
                                        <InputNumber disabled v-model="withdrawFee" :min="currentCoin.minTxFee" :max="currentCoin.maxTxFee" size="large" style="width: 100%;"></InputNumber>
                                ...
                                <div class="order-table">
                                    ...
                                    <div style="margin: 10px;overflow: hidden">
                                        <div style="float: right;">
                                            <Page :total="transaction.total" :pageSize='transaction.pageSize' :current="transaction.page + 1" @on-change="changePage"></Page>
                                        </div>
                                    </div>
                                </div>
                            </div>
      ...
      <Modal v-model="modalGoggle" :title="$t('otc.chat.tip')" @on-ok="ok1">
        <P style="color:red;font-weight: bold;">
          {{$t('uc.safe.googelCode')}}<br/>
          <Input  v-model="googelCode" :placeholder="$t('uc.safe.googelRequired')"></Input>
        </p>
      </Modal>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        ok1(){
            if (this.googelCode == "") {
                this.$Message.error(this.$t('uc.safe.googelRequired'));
                return;
            }
            this.$http.get(this.host + '/uc/google/yzgoogle'+"?codes="+this.googelCode).then(response => {
                this.googelCode = "";
                var resp = response.body;
                if (resp.code == 0) {
                    this.$Message.success(resp.message);
                    this.modalGoggle=false;
                    this.modal=true;
                }else {
                    this.$Message.error(resp.message);
                }
          ...
          this.$http.post(this.host + '/uc/approve/security/setting').then(response => {
            ...
            if (resp.code == 0) {
                if (resp.data.realName == null || resp.data.realName == "") {
                    this.$Message.info(this.$t('otc.publishad.submittip1'));
                    self.$router.push('/uc/safe');
                } else if (resp.data.googleStatus != 1) {
                    this.$Message.info(this.$t('uc.safe.googelBind'));
                    self.$router.push('/uc/safe');
                } else if (resp.data.fundsVerified == 0) {
                    this.$Message.info(this.$t('otc.publishad.submittip3'));
                    self.$router.push('/uc/safe');
                }
            } else {
              this.$Message.error(resp.message);
