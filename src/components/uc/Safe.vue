                    ...
                    <div class="account-in">
                        ...
                        <div class="account-item">
                            <div class="account-item-in">
                                ...
                                <a class="btn" v-else-if="user.googleStatus!=1"  @click="noGoogle()">{{$t('uc.safe.edit')}}</a>
                            ...
                            <div class="account-detail" v-show="choseItem==5">
                                ...
                                <div class="detail-list"  v-show="user.fundsVerified==1 && !google">
                                     <!-- 谷歌验证 -->
                                    <Form ref="formValidateGoogle" :model="formValidateGoogle" :rules="ruleValidate" :label-width="85">
                                    
                                        <FormItem :label="$t('uc.safe.googelCode')" prop="identifyingCode">
                                                <Input v-model="formValidateGoogle.identifyingCode" size="large" :placeholder="$t('uc.safe.googelRequired')"></Input>
                                            </FormItem>
                                            <FormItem>
                                            <Button type="primary" @click="handleSubmit('formValidateGoogle')">{{$t('uc.safe.save')}}</Button>
                                            <Button type="ghost" @click="handleReset('formValidateGoogle')" style="margin-left: 8px">{{$t('uc.safe.reset')}}</Button>
                                        </FormItem>
                                    </Form>   
                                </div>
                                <!-- 修改 -->
                                <!-- <div class="detail-list" v-show="user.fundsVerified==1  && !fGetBackFundpwd "> -->
                                <div class="detail-list" v-show="user.fundsVerified==1  && !fGetBackFundpwd && google">
                                    <Form ref="formValidate5" :model="formValidate5" :rules="ruleValidate" :label-width="85">
                                        ...
                                        <!--<FormItem :label="$t('uc.safe.phonecode')" prop="vailCode5">-->
                                            ...
                                            <!--</Input>-->
                                        <!--</FormItem>-->
                                        <p style="text-align:right;"><a @click="handleReset('formValidate8');fGetBackFundpwd=!fGetBackFundpwd">{{$t('uc.login.forget')}}?</a></p>
                                        <!-- Button -->
                                        <FormItem>
                        ...
                        <div class="account-item" >
                            <div class="account-item-in">
                                <Icon type="lock-combination" style="font-size: 18px;color: #00b5f6;"></Icon>
                                <span class="card-number">{{$t('uc.safe.googel')}}</span>
                                <p class="bankInfo" style="color: grey;">
                                    {{$t('uc.safe.googelTip')}}
                                </p>
                                <a class="btn" v-if="user.googleStatus!=1" @click="showItemGoogel()">{{$t('uc.safe.bind')}}</a>
                                <a class="btn" v-else  @click="deleteItemGoogel()">{{$t('uc.safe.unBind')}}</a>
                            </div>
                            <div class="account-detail" v-show="choseItem==9">
                                <div class='googel'>
                                    <div class='qrCode'>
                                    ...
                                    <div class='secretKey'>
                                        <!-- <P> {{$t('uc.safe.googelTip1')}}</P> -->
                                        
                                        <p>{{$t('uc.safe.account')}}：{{secretAccount}}<a v-clipboard:copy="secretAccount" v-clipboard:success="onCopy" v-clipboard:error="onError" href="javascript:;" id="copyBtn" class="link-copy">{{$t('uc.finance.recharge.copy')}}</a></p>
                                        <p>{{$t('uc.safe.secret')}}：{{secret}}<a v-clipboard:copy="secret" v-clipboard:success="onCopy" v-clipboard:error="onError" href="javascript:;" id="copyBtn" class="link-copy">{{$t('uc.finance.recharge.copy')}}</a></p>
                                    </div>
                                </div>
                                <div class="detail-list" >
                                     <Form ref="formValidate9" :model="formValidate9" :rules="ruleValidate" :label-width="85">
                                        <FormItem :label="$t('uc.safe.googelCode')" prop="identifyingCode">
                                            <Input v-model="formValidate9.identifyingCode" size="large"  :placeholder="$t('uc.safe.googelRequired')"></Input>
                                        </FormItem>
                                        <FormItem>
                                            <Button type="primary" @click="handleSubmit('formValidate9')">{{$t('uc.safe.save')}}</Button>
                                        </FormItem>
                                    </Form>
                                </div>
                            </div>
                            <div class="account-detail" v-show="choseItem==10">
                                <div class="detail-list" >
                                    <Form ref="formValidate10" :model="formValidate10" :rules="ruleValidate" :label-width="85">
                                         
                                        <FormItem :label="$t('uc.regist.pwd')" prop="loginPassword">
                                            <Input v-model="formValidate10.loginPassword" type="password" size="large" :placeholder="$t('uc.regist.pwdtip')"></Input>
                                        </FormItem>
                                        <FormItem :label="$t('uc.safe.googelCode')" prop="identifyingCode">
                                            <Input v-model="formValidate10.identifyingCode" size="large"  :placeholder="$t('uc.safe.googelRequired')"></Input>
                                        </FormItem>
                                        <FormItem>
                                            <Button type="primary" @click="handleSubmit('formValidate10')">{{$t('uc.safe.save')}}</Button>
                                        </FormItem>
                                    </Form>
                                </div>
                                <div class="detail-list" v-show="user.fundsVerified==1  && !fGetBackFundpwd"></div>
                            </div>
                        </div>
                    </div>
                </section>
...
export default {
    ...
    data() {
        ...
        return {
            ...
            ruleValidate: {
                ...
                identifyingCode:[
                    { required: true, message: this.$t('uc.safe.googelRequired'), type: 'string', min: 6, trigger: 'blur' },
                ],
                loginPassword:[
                     { required: true, message: this.$t('uc.regist.pwdtip'), type:'string'},
                ]
            },
            time1: 60, // 发送验证码倒计时
            time2: 60, // 发送验证码倒计时
    ...
    methods: {
        frontHandleSuccess (res, file) {
            this.frontCardImg=this.imgPreview=this.url+res.data;
        },
        backHandleSuccess (res, file) {
            this.backCardImg=this.imgNext=this.url+res.data;
        },
        handHandleSuccess (res, file) {
            this.handCardImg=this.imgLast=this.url+res.data;
        },
        noPhone() {
        ...
        noGoogle() {
            this.$Message.info(this.$t('uc.safe.googelBind'));
            this.showItem(9);
        },
        showItemFundpwd() {
            this.fGetBackFundpwd = false;
            this.google = false;
            this.formValidateGoogle.identifyingCode='';
        ...
        deleteItemGoogel(){
            this.handleReset('formValidate10');
            this.showItem(10);
        },
        renderPw() {
            this.$Modal.confirm({
        ...
        submit(name) {
            ...
            if (name == 'formValidate9') {
                let param = {}
                param['codes'] = this.formValidate9.identifyingCode;
                param['secret'] = this.secret;
                this.$http.post(this.host + '/uc/google/googleAuth', param).then(response => {
                    var resp = response.body;
                    if (resp.code == 0) {
            ...
            if (name == 'formValidate10') {
                let param = {}
                param['codes'] = this.formValidate10.identifyingCode;
                param['password'] = this.formValidate10.loginPassword;
                this.$http.post(this.host + '/uc/google/jcgoogle', param).then(response => {
                    var resp = response.body;
                    if (resp.code == 0) {
                        this.$Message.success(resp.message);
                        this.getMember()
            ...
            // 谷歌验证
            if (name == 'formValidateGoogle') {
                this.$http.get(this.host + '/uc/google/yzgoogle'+"?codes="+this.formValidateGoogle.identifyingCode).then(response => {
                    var resp = response.body;
                    if (resp.code == 0) {
                        this.$Message.success(resp.message);
                        this.google=true;    
        ...
        isGoogel(){
            var self = this;
            this.$http.get(this.host + '/uc/google/sendgoogle').then(response => {
                var resp = response.body;
                if (resp.code == 0) {
                    this.qrcode.value=resp.data.link;
                    this.secret=resp.data.secret;
                    this.secretAccount=(resp.data.link).split('otpauth://totp/')[1].split('?secret')[0];
