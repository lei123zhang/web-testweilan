            ...
            <Tabs :value="tabPage" v-model="tabPage">
                <TabPane :label="$t('otc.buyin')" name="buy">
                    ...
                    <div class="table-responsive list-table">
                        <Table :no-data-text="$t('common.nodata')" :border="showBorder" :stripe="showStripe" :show-header="showHeader" :height="fixedHeader ? 250 : ''" :size="tableSize" :data="advertiment.ask.rows" :columns="advertiment.columns" :loading="loading"></Table>
                ...
                <TabPane :label="$t('otc.sellout')" name="sell">
                   <div class="table-responsive list-table">
                        <Table :no-data-text="$t('common.nodata')" :border="showBorder" :stripe="showStripe" :show-header="showHeader" :height="fixedHeader ? 250 : ''" :size="tableSize" :data="advertiment.bid.rows" :columns="advertiment.columns" :loading="loading"></Table>
                        <div style="margin: 10px;overflow: hidden">
                            <div style="float: right;">
            ...
            advertiment:{
                ...
                columns:[
                    ...
                    },
                    {
                        ...
                        render: function(h, params) {
                            let txt='';
                            let payMode=params.row.payMode.split(',');
                            for(let i=0;i<payMode.length;i++){
                                if( payMode[i]=='微信'){
                                    if(txt!=''){
                                        txt+=','+self.$t('otc.publishad.wx')
                                    }else{
                                        txt+=self.$t('otc.publishad.wx')
                                    }
                                    
                                }else  if(payMode[i]=='银联'|| payMode[i]=='银行卡'){
                                    if(txt!=''){
                                        txt+=','+self.$t('otc.publishad.unionpay')
                                    }else{
                                        txt+=self.$t('otc.publishad.unionpay')
                                    }
                                   
                                } else  if(payMode[i]=='支付宝'){
                                    if(txt!=''){
                                        txt+=','+self.$t('otc.publishad.zfb')
                                    }else{
                                        txt+=self.$t('otc.publishad.zfb')
                                    }
