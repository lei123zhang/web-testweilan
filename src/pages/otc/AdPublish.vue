...
export default {
    data() {
        ...
        const minCheck = (rule, value, callback) => {
            ...
            if (this.form.maxLimit && this.form.maxLimit != 0 && parseFloat(value) > (this.form.maxLimit - 0)) {
    ...
    methods: {
        changeCoin(){
            ...
            if(coinItem != null) {
                ...
                // console.log(this.cankao,lv,cankoNew,this.price)
        ...
        getMember() {
            ...
            this.$http.post(this.host+'/uc/approve/certified/business/status').then(res=>{
                if(res.body.code==0){
                    if(res.body.data.certifiedBusinessStatus!=2){
                        this.$Message.info(this.$t('otc.identBusiness.applyIdentBusiness'));
                         self.$router.push('/identbusiness');
    ...
    watch: {
        ...
        premisePriceHistory(newValue, oldValue) {
            ...
            if(!this.isId){
                //  this.price = this.round(this.mul(cankoNew, lv), 2).toLocaleString() + ' CNY/' + this.findCoin(this.form.coin);
                 self.price = this.round(this.mul(cankoNew, lv), 2).toLocaleString();
