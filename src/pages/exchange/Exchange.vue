<template>
    <div class="container exchange">
        <div class='container_top'>
            <div class="sidebar">
                <div v-if="isLogin" style="" class="hd hd_login">
                ...
                <Table @on-current-change="gohref" highlight-row id="USDT" v-show="true" :columns="coins.columns" :data="coins.USDT" :no-data-text="$t('common.nodata')" ></Table>
                <Table @on-current-change="gohref" highlight-row id="BTC" v-show="false" :columns="coins.columns" :data="coins.BTC" :no-data-text="$t('common.nodata')" ></Table>
                <Table @on-current-change="gohref" highlight-row id="ETH" v-show="false" :columns="coins.columns" :data="coins.ETH" :no-data-text="$t('common.nodata')" ></Table>
                <Table :no-data-text="$t('common.nodata')"  id="collect" v-show="false" :columns="coins.columns" :data="coins.favor"></Table>
                <div class="deal_record">
                    <div class="deal_record_hd">
                        {{$t("exchange.realtransaction")}}
                    </div>
                    <Table height="" :columns="trade.columns" :data="trade.rows" :no-data-text="$t('common.nodata')" ></Table>
                </div>
            </div>
            <div class="kline">
                ...
                <div class='mod_bottom'>
                        ...
                        <div class="trade_panel trade_panel_logout">
                            ...
                            <div class="trade_bd">
                                <div class="panel panel_buy">
                                    ...
                                    <div class="bd bd_limited" v-show="!showMarket">
                                        <Form ref="formValidate">
                                            <p style="font-size:10px;color:#589065;">
                                                {{$t("exchange.buyprice")}}
                                            </p>
                                            <FormItem>
                                            ...
                                            <Button @click="buyWithLimitPrice" v-show="isLogin" style='background-color:#589065'>{{$t("exchange.buyin")}}{{currentCoin.coin}}</Button>
                                        </Form>
                                    </div>
                                    <div class="bd bd_market" v-show="showMarket">
                                        <Form ref="formValidate">
                                            ...
                                            <Button @click="buyWithMarketPrice" v-show="isLogin" style='background-color:#589065'>{{$t("exchange.buyin")}}{{currentCoin.coin}}</Button>
                                        </Form>
                                    </div>
                                </div>
                                <div class="panel panel_sell">
                                    ...
                                    <div class="bd bd_limited" v-show="!showMarket">
                                        <Form ref="formValidate">
                                            <p style="font-size:10px;color:#AE4E54;">
                                            ...
                                            <Button @click="sellLimitPrice" v-show="isLogin" style='background-color:#AE4E54;'>{{$t("exchange.sellout")}}{{currentCoin.coin}}</Button>
                                        </Form>
                                    </div>
                                    <div class="bd bd_market" v-show="showMarket">
                                        <Form ref="formValidate">
                                            ...
                                            <!--<Slider v-model="value6" :step="25" show-stops min="0" max="100"></Slider>-->
                                            <!--</FormItem>-->
                                            <Button @click="sellMarketPrice" v-show="isLogin" style='background-color:#AE4E54;'>{{$t("exchange.sellout")}}{{currentCoin.coin}}</Button>
                                        </Form>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="order_book">
                        ...
                        <Table  @on-current-change="buyPlate" highlight-row ref="currentRowTable" class="sell_table" :columns="plate.columns" :data="plate.askRows" :no-data-text="$t('common.nodata')" ></Table>
                        <Table  @on-current-change="sellPlate" highlight-row class="buy_table" :columns="plate.columns" :data="plate.bidRows" :no-data-text="$t('common.nodata')" ></Table>
                    </div>
                </div>
        ...
        <!-- <div class="trade_wrap"></div> -->
        <div class="open_orders" v-show="isLogin">
             <div class='open_orders_th'>{{$t('exchange.curdelegation')}}</div>
            <!-- <Tabs>
               
                <TabPane > -->
                    <Table  :columns="currentOrder.columns" :data="currentOrder.rows" :no-data-text="$t('common.nodata')" ></Table>
                <!-- </TabPane>
                
            </Tabs> -->
        </div>
        <div class="open_orders" v-show="isLogin"> 
             <div class='open_orders_th'>{{$t('exchange.hisdelegation')}}</div>   
            <!-- <Tabs>
               
                <TabPane > -->
                    <!-- <Table :height="fixHistoryHeight"  :columns="historyOrder.columns" :data="historyOrder.rows"></Table> -->
                    <Table :columns="historyOrder.columns" :data="historyOrder.rows" :no-data-text="$t('common.nodata')" ></Table>
            ...
            plate:{
                ...
                columns:[
                    ...
                    },
                    {
                        ...
                        key: 'price',
                        render:(h,params)=>{
                            const price =  (params.row.price==0)?'--':params.row.price.toFixed(this.baseCoinScale);
                            return h('span',{
                            },price)
                        },
                        renderHeader:(h,params)=>{
        ...
        getHistoryOrder(pageNo){ //查询历史委托
            ...
            this.$http.post(this.host+this.api.exchange.history,params).then(response => {
                ...
                if(resp.content.length>0){
                    ...
                    // console.log(resp.content,resp.content.length)
                    for(let i=0;i<resp.content.length;i++){
                   
                        let row = '';
                        row=resp.content[i];
                        row.price = row.type=='MARKET_PRICE'?that.$t('exchange.marketprice'):row.price;
                        if(i>=this.historyOrder.rows.length){
                             this.historyOrder.rows.push(row);
                        }
                       
                        // console.log('row',row)
                        // console.log('historyOrder.rows',this.historyOrder.rows);
                    }
                }
            });
        },
